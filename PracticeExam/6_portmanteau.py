...
def natalie(words):
    ...
    prefixes = get_prefixes(words)
    suffixes = get_suffixes(words)
    ports = [(beg[:beg.find(mid)], mid, end[len(mid):])
             for mid in prefixes & suffixes
             for beg in words if beg.endswith(mid)
             for end in words if beg != end and end.startswith(mid)]
    return ''.join(max(ports, key=score)) if ports else None

def get_prefixes(words):
    return set([w[:i] for w in words for i in range(1, len(w)) if w])
...
def score(port):
    beg, mid, end = port
    w_len = len(beg) + len(mid) + len(end)
    return w_len - abs(len(beg)-w_len/4) - abs(len(mid)-w_len/2) - abs(len(end)-w_len/4)
