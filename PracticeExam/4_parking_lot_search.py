...
def is_goal(state):
    for item in state:
        if item[0] == '*': car  = set(item[1])
        if item[0] == '@': exit = set(item[1])
    return bool(car & exit)

def successors(state):
    res = {}
    for item in state:
        if item[0] in ('|', '@'): continue
        offset = abs(item[1][1] - item[1][0])
        res.update(moves(state, item, offset))
        res.update(moves(state, item, -offset))
    return res

def moves(state, item, offset):
    res = {}
    move = offset
    while 1:
        action = (item[0], move)
        state2 = apply(action, state)
        if legit(state2):
            res[state2] = action
        else:
            break
...
def apply(action, state):
    ...
    for item in state:
        if item[0] == action[0]:
            res.append((item[0], tuple(i+action[1] for i in item[1])))
        else:
            res.append(item)
    return tuple(res)

def legit(state):
    objects = [set(item[1]) for item in state if item[0] != '@']
    return not any(set.intersection(*pair) for pair in combinations(objects, 2))
...
def locs(start, n, incr=1):
    ...
    return tuple(i for i in range(start, start + incr*n, incr))
...
def grid(cars, N=N):
    ...
    exit = ((N**2-1)//2, )
    walls = tuple(i for i in range(N**2) if i//N in (0, N-1) or i%N in (0, N-1) and i != exit[0])
    return (('|', walls), ('@', exit)) + cars
...
def show(state, N=N):
    ...
    for i,s in enumerate(board):
        sys.stdout.write(s+' ')
        if i % N == N - 1: sys.stdout.write('\n')

def show_path(path):
    for s in path_states(path):
        show(s)
        sys.stdout.write('\n')
...
def path_states(path):
    "Return a sequence of states in path."
    return path[::2]
