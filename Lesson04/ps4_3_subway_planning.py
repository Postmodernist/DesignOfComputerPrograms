...
def subway(**lines):
    ...
    res = collections.defaultdict(dict)  # defaults non-existent key to empty dict
    for line in lines:
        stations = lines[line].split()
        for station, nbs in zip(stations, neighbors(stations)):
            res[station].update({nb:line for nb in nbs if nb is not None})
    return res


def neighbors(items):
    """Make a list of lists of neighbors of each item in items."""
    return zip([None] + items[:-1], items[1:] + [None])
...
def longest_ride(system):
    ...
    successors = lambda x: system[x]
    routes     = it.combinations(system.keys(), 2)
    longest    = []
    for route in routes:
        path = ride(route[0], route[1], system)
