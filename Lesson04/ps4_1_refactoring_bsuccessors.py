...
def bsuccessors3(state):
    ...
    res = {}
    src, dst, dir = (there, here, '<-') if light else (here, there, '->')
    groups = list((x, x) for x in src) + list(it.combinations(src, 2))

    for grp in groups:
        actn = (set(grp), dir)
        src2 = src - frozenset(grp)
        dst2 = dst | frozenset(grp)
        if light:
            res[(dst2, src2, light^1)] = actn
        else:
            res[(src2, dst2, light^1)] = actn
    return res
