...
def strategy_d(state):
    ...
    def EU(action):
        return Q_pig_d(state, action)
    return max(pig_actions_d(state), key=EU)

def Q_pig_d(state, action):
    """The expected value of choosing action in state."""
    if action == 'roll':
        return (-U_pig_d(do(action, state, iter([1]))) + \
            sum(U_pig_d(do(action, state, iter([d]))) for d in range(2, 7))) / 6.0
    if action == 'double':
        if state[1] > goal*0.6:
            return -U_pig_d(do(action, state, None))
        else:
            return -1
    if action in ['hold', 'decline', 'accept']:
        return -U_pig_d(do(action, state, None))
    raise ValueError
...
def U_pig_d(state):
    ...
    (p, me, you, pending, double) = state 
    dbl = 1 if double == 'double' else double
    if me + pending >= goal:
        return dbl
    if you >= goal:
        return -dbl
