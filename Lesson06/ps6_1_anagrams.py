...
def anagrams(phrase, shortest=2):
    ...
    res = set()
    find_anagrams_r(phrase.replace(' ', ''), [], res, shortest)
    return res

def find_anagrams_r(letters, pre, res, shortest):
    words = [w for w in find_words(letters) if not pre or w >= pre[-1]] \
            if len(letters) >= shortest else []
    for w in words:
        letters_rest = removed(letters, w)
        pre_next = pre[:]
        pre_next.append(w)
        if not letters_rest:  # add combination to results
            res.add(' '.join(pre_next))
        else:                 # continue to search
            find_anagrams_r(letters_rest, pre_next, res, shortest)
