"""
Scrabble is a word game in which two to four players score points by placing tiles
...
def best_play(hand, board):
    ...
    return max(plays) if plays else NOPLAY
...
def set_anchors(row, j, board):
    ...
    for (i, sq) in enumerate(row[1:-1], 1):
        ...
        if is_empty(sq) and (BONUS[j][i] == '*' or any(map(is_letter, neighborlist))):
...
def show(board):
    ...
    for j,row in enumerate(board):
        print(' '.join((sq if sq == '|' or is_letter(sq) else BONUS[j][i])
