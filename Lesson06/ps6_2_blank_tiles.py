...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    if pre.upper() in PREFIXES:
        ...
        if is_letter(sq):
            ...
            for L in hand:
                if L == '_':
                    hand_rest = hand.replace('_', '', 1)
                    for l in ''.join(possibilities).lower():
                        add_suffixes(hand_rest, pre+l, start, row, results)
    ...
    if pre.upper() in PREFIXES:
        for L in hand:
            if L == '_':
                hand_rest = hand.replace('_', '', 1)
                for l in LETTERS_L:
                    find_prefixes(hand_rest, pre+l, results)
            else:
...
def calculate_score(board, pos, direction, hand, word):
    ...
    for (n, L) in enumerate(word):
        ...
        total += (POINTS[L] if L.isupper() else 0) * letter_mult
