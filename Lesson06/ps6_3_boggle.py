...
def boggle_words(board, minlength=3):
    ...
    for i in range(len(board)):
        if board[i] == BORDER: continue
        find_words_r(board, [i], minlength, result)
    return result

def find_words_r(board, path, minlength, result):
    w = ''.join(board[i] for i in path)
    if len(w) >= minlength and w in WORDS:
        result.add(w)
    if w not in PREFIXES:
        return
    # continue search
    next_steps = [i for i in neighbors(path[-1], size(board))
                  if board[i] != BORDER and i not in path]
    for i in next_steps:
        find_words_r(board, path+[i], minlength, result)
