...
def find_prefixes_norvig(hand, pre='', results=None):
...
hand_cache = defaultdict(set)

def find_prefixes(hand, pre='', results=None):
    """Find all prefixes (of words) that can be made from letters in hand."""

    def find_prefixes_r(hand, pre):
        if pre in WORDS or pre in PREFIXES: results.add(pre)
        if pre in PREFIXES:
            results.add(pre)
            for L in hand:
                find_prefixes_r(hand.replace(L, '', 1), pre+L)

    if hand in hand_cache:
        return hand_cache[hand]
    else:    
        if results is None: results = set()
        hand_cache[hand] = results
        find_prefixes_r(hand, pre)
        return results
